...
check.if.zs.calc.df.ftp <- function(df,var.name,zs_stat) {
  
  if (! zs_stat %in% c('sum', 'mean', 'min', 'max')) {
    stop("zs_stat argument in function check.if.zs.calc.df.ftp can be 'sum', 'mean', 'min' or 'max'")
  }  
  ...
  ##  JJN:  Changing this to allow for non standard covariates which are passed in to be allowed past this section without tossing issues.
  if( nrow(df[df$CvtName == var.name, ]) == 0 ) {
    return(FALSE)
  }
  
  if ( is.null(df[df$CvtName == var.name, ][[select_stat]] ) ) {
    return(FALSE)
  }
  
  if ( df[df$CvtName == var.name, ][[select_stat]]  == FALSE)  {
    return(FALSE)
  }  
...
calculate.zonal.stats.country <- function(icvr,icountry,zonal_raster){
  ...
  if(!file.exists(file.path.csv )){
    ...
    # if not then > else
    if (check.if.zs.calc.df.ftp(df.ftp,var_name_class,"mean") & is.null(bsgm.input.shp)){   
      ...
      if ( bsgm.kPythonZonal ) {
        ...
        minblks <- wpGetBlocksNeed (dataset_raster,bsgm.cluster_workers, n=2)
