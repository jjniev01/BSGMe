...
##  INITIAL HIST. AVG. SQR GROWTH/DECAY FITTING FUNCTION  ----------------------
fitter.SQR.init <- function(ints, k_value, h_value=1, testmode = "rolling"){
  #  ints is an input timeseries for a single GID.
  ##  k_value is the size of the training set desired minus 1
  ...
  if(testmode == "rolling"){
    ...
    for(i in 0:k_value){
      ...
      testdf <- data.frame("VAL" = as.numeric(ints[1:{k_value+i}]),
                           "TIME" = sqrt(1:{k_value+i}))
      ##  Define our holdout values
      holdout_ts <- ints[{length(ints)-{k_value+i-1}}:{length(ints)}]
      
      ##  Least Squares LOG NORMAL best Fit:
      fooSQR <- glm(VAL~TIME, data = testdf)
      ...
      foo_fore <- predict(fooSQR,data.frame("TIME"={sqrt({length(ints)-{k_value+i-1}}:{length(ints)})}))
      mape_holder[i+1] <- 
  ...
  if(testmode == "multistep"){
    ...
    testdf <- data.frame("VAL" = as.numeric(ints[1:{k_value+1}]),
                         "TIME" = sqrt(1:{k_value+1}))
    ##  Least Squares LOG NORMAL best Fit:
    fooSQR <- glm(VAL~TIME, data = testdf)
    
    ##  Predict next h steps using the average rate of change:
    foo_fore <- predict(fooSQR, data.frame("TIME"={sqrt({k_value+1}:{k_value+h_value})}))
    mape <- 100 *
      sum(abs({as.numeric(holdout_ts)-foo_fore}/as.numeric(holdout_ts)))
    return(mape)
  }
...
##  FINAL SQRT REGRESSION FITTING FUNCTION  ------------------------
fitter.SQR.final <- function(ints){
  ##  Put into data.frame for the glm function:
  foodf <- data.frame("VAL" = as.numeric(ints), 
                      "TIME" = sqrt(1:length(ints)))
  ##  Fit the LS SQRT line of best fit through the observed data:
  fooSQR <- glm(VAL~TIME, data = foodf)
  ##  Return the rate:
  return(fooSQR)
}
...
##  SQR REDUCTION FUNCTION  ----------------------------------------------------
reduce.SQR <- function(object){
  ...
  sqrstring <- paste0("Square root regression with intercept of ",
                      intercept_str,
                      " and time coefficient of ",
                      timecov_str)
  return(sqrstring)
}
...
final.time.model.select <- function(arimamape = 10000, 
  ...
  if(!is.numeric(sqrmape) | is.na(sqrmape) | is.null(sqrmape)){sqrmape <- 10000}
  
  ##  List final model tags in order:
  finmods <- c("ETS","ARIMA","SQR")
  ...
  ##  ties occur:
  allmapes <- c(etsmape,arimamape,sqrmape)
...
time.model.fit.prl <- function(i,
  ...
  best.sqr <- fitter.SQR.init(ts_full, k_value = k, h_value = h, 
                              testmode = test_mode)
  ...
  if(final_model_type == "SQR"){
    ##  Get the exponential (log normal) model rate:
    final_model <- fitter.SQR.final(ts_full)
    ##  Get the model descriptive string:
    final_model_desc <- reduce.SQR(final_model)
    ##  Produce a prediction for the number of desired/indicated time steps and 
    ##  place it in the object which is being returned from this function:
    predictframe <- data.frame("TIME" = {sqrt(length(ts_full)+1)}:{sqrt(length(ts_full)+bsgm.predict.steps)})
