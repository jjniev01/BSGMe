...
create_dirs_for_prj <- function(){
  ...
  ##  Configure the folders for the old RF objects that we will retrieve:
  data.path.old.bsgmfits <- paste0(root_path, "/data/",bsgm.input.countries,"/old_bsgmfits/")
  ##  If that folder does not already exist, create it:
  if(!file.exists(data.path.old.bsgmfits)){ 
    dir.create(data.path.old.bsgmfits, recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0(data.path.old.bsgmfits, "bsgmfits_final/"), recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0(data.path.old.bsgmfits, "bsgmfits_quant/"), recursive = TRUE, showWarnings = FALSE)
    logdebug(paste0('Created a folder: ',data.path.old.bsgmfits))
  }      
  ...
  return(list(output = output.path.countries, 
              ...
              data_old_bsgmfits_final = paste0(data.path.old.bsgmfits, "bsgmfits_final/"),
              data_old_bsgmfits_quant = paste0(data.path.old.bsgmfits, "bsgmfits_quant/")
  ))
