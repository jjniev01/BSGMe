...
require(precrec)
require(scales)
source("D:/Research/BSGMe/accessories/Geoprocessing_Functions.R")
...
iso3 <- c("CHE","PAN","UGA","VNM")
##  Comparison years:
years <- c(2001:2004,2006:2009)
year_tag <- "2000-2005-2010"
data_tag <- "esa_cls190"
for(i in iso3){
...
write.csv(foodf, 
          file = paste0(root, "CountryLevelPixelComparisons_2000-2005-2010.csv"),
          row.names = F)
##  Add the country level null data to it:
foodf <- as.data.table(foodf)
for(i in 1:length(iso3)){
  print(iso3[i])
  foonull <- as.data.table(readRDS(paste0(root,"output/prj_",year_tag,"_",iso3[i],
                                          "/derived/",iso3[i],
...
model_list <- list("2000-2005-2010" = list("1"=c(2000,2005),
                                           "2"=c(2005,2010)))
                                                # "3"=c(2010,2015)))
...
write.csv(foodt, 
          file = paste0(root, "AdminLevelPixelComparisons_2000-2005-2010.csv"),
          row.names = F)
