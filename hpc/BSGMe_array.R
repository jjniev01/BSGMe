...
root_path <- "/scratch/jjn1n15/BSGMe"
...
##  First argument should be a character string consisting of the 3 letter ISO:
bsgm.input.countries <- as.list(eval(parse(text=args[1])))
##  Beginning year of the interpolation period, e.g. '2012':
t0 <- as.numeric(eval(parse(text=args[2])))
##  End year of the interpolation period, e.g. '2014':
t1 <- as.numeric(eval(parse(text=args[3])))

nmb <- as.numeric(eval(parse(text=args[4])))

##  Load the model input script and get an idea about what countries we are 
##  working with:
source(paste0(root_path,"/inpute_array.R"))
...
source(paste0(root_path,"/confige_array.R"))
...
if(!file.exists(paste0(root_path,"/data/",bsgm.input.countries,"/",
                       bsgm.input.countries,"_POP_BSCNT_BSPOP_2000_14.csv")) &
...
t0extentsPathFileName <- covariates[[bsgm.t0.extents]]$path
...
##  BEGIN:  FITTING OF ARIMA/EXS BY SUBUNIT  -----------------------------------
...
####
##  Get a list of all the GIDs we will be processing through.
gid_list <- unique(hist_pop$GID)
...
urr_forecasts[URR.MOD.TAG == "LIN" | URR.MOD.TAG == "EXP", c("URR.H95", "URR.L95") := NA] 
...
##  Convert any linear and exponetial typoe model H95 and L95 valeus to NA
bspdbar_forecasts[BSPDBAR.MOD.TAG == "LIN" | BSPDBAR.MOD.TAG == "EXP", c("BSPDBAR.H95", "BSPDBAR.L95") := NA] 
...
for(t in 2:(length(year_seq))){
  ...
  lan_path <- Sys.glob(paste0(bsgm.data.path.countries, "/LAN/derived/*",
                              lan_pattern,{y-1},"*.tif"))[1]
