...
rfValidate <- function(root = "E:/",
  ...
  trans_ras <- raster(list.files(paste0(prj_fold,"/tmp/"),
                                 pattern = paste0("prj_",model_tag,"_",iso,"_transition_2000_2010.tif"),
                                 full.names = TRUE))
...
##  For very country:
for(i in iso3){
  outdir <- ensure_dir(paste0(root,"output/prj_",model_tag,"_",i,"/derived/"))
...
for(i in iso3){
  ...
  ##  Get the number of cells for every GID:
  ...
  ##  Determine which cell indices are valid:
  ...
  for(y in years){
    ...
    predfile_path <- paste0(preddir, "BSGM_Extentsprj_",year_tag,"_",i,"_",y,".tif")
    ...
    foo_table <- data.table("CID" = pred_1_ind,
                            ...
                            "PRED" = pred_ras[pred_1_ind],
                            "OBS" = obs_ras[pred_1_ind])
    
    foo_2 <- data.table("CID" = obs_1_ind,
                        "GID" = region_ras[obs_1_ind],
                        "PRED" = pred_ras[obs_1_ind],
                        "OBS" = obs_ras[obs_1_ind])
    ...
    foo_table <- rbind(copy(foo_table),
                       list(99999999,99999999,0,0,0,0,
                            {tot_valid_cells - sum(foo_table$TP,
                                                   foo_table$FP,
                                                   foo_table$FN,
                                                   na.rm = T)},
                            0))
    ...
    ##  Derive the TN for each GID:
    aggtab[,TN := {N.TOT-TP-FP-FN},by=GID]
...
for(l in 1:length(iso3)){
  ...
  preddir <- paste0(root,in_series," Input Validation/prj_",year_tag,"_",i,"/")
  ...
  for(y in years){
    ...
    cell_n <- sum(dt$TP,dt$FP,dt$FN,max(dt$TN))
...
write.csv(foodf, 
          file = paste0(root, "CountryLevelPixelComparisons_Extrapolation_",
                        in_series,"_2010-2015.csv"),
          row.names = F)
##  Add the country level null data to it:
foodf <- as.data.table(read.csv(paste0(root, "CountryLevelPixelComparisons_Extrapolation_",
                                       in_series,"_2010-2015.csv"),
                                header = T, stringsAsFactors = F))
for(i in 1:length(iso3)){
  print(iso3[i])
  ##  Load in the null data 
  foonull <- as.data.table(readRDS(paste0(root,in_series,
                                          " Input Validation/prj_",year_tag,"_",
                                          iso3[i],"/derived/",iso3[i],
                                          "_esa_cls190NULLMODEL_admin_level_contingency_data_2010-2015.RDS")))
  ##  Aggregate it:
  years <- copy(foonull$YEAR)
  foonull[,GID:=NULL]
  foonull[,YEAR:=NULL]
  #foonull[,ISO:=NULL]
  ...
  ##  Aggregate to the subnational unit level and join to the shapefile we 
  ##  brought in earlier:
  aggnull <- as.data.table(aggregate.data.frame(foonull,
                                               by = list(years),
                                               simplify = T,
                                               FUN = sum, 
                                               na.rm = T))
  ...
  # foonull$GID <- as.numeric(foonull$GID)
  names(aggnull)[1] <- "YEAR"
  aggnull$YEAR <- as.numeric(aggnull$YEAR)
  foodf$YEAR <- as.numeric(foodf$YEAR)
  aggnull$ISO <- iso3[i]
  ##  Merge the null model results by year and GID with the data:
  setkey(foodf,ISO,YEAR)
  setkey(aggnull,ISO,YEAR)

  ##  Change the names to the desired format:
  #names(foonull)[c(3,4,5,6)] <- c("TP.NULL","FN.NULL","FP.NULL","TN.NULL")
  
  ##  Merge the columns over:
  foodf[aggnull,
        names(aggnull)[c(4,5,6,7)] := aggnull[,.SD,.SDcols = c(4,5,6,7)]]
  # write.csv(foonull, file = paste0(root,"output/prj_",year_tag,"_",iso3[i],
  #                                  "/derived/",iso3[i],
  #                                  "_esa_cls190_NullBootCountrywidebyYear_500_",
  #                                  year_tag,".csv"),
  #           row.names = F)
}

##  Calculate other metrics for both the modeled and null model:
foodf[,c("FPR",
         "TPR",
         "PPV",
         "FOR",
         ...
         "logDOR") := .(FP/{FP+TN}, #FPR
                        ...
                        TN/{TN+FN}, #NPV
                        FP/{FP+TP}, #FDR
                        {{TP/{TN+FP}}/{FP/{FP+TN}}}, #PLR
                        FN/{FN+TP}, #FNR
                        {{FN/{FN+TP}}/SPECIFICITY}, #NLR
                        {{{TP}*{TN}}/{{FN}*{FP}}}, #DOR
                        0)] #logDOR holder

foodf[,logDOR:=log({DOR})]

##  Do the same for the null data:
##  Calculate other metrics for both the modeled and null model:
foodf[,c("FPR.NULL",
         "TPR.NULL",
         "PPV.NULL",
         ...
         "NLR.NULL",
         "DOR.NULL",
         "logDOR.NULL") := .(FP.NULL/{FP.NULL+TN.NULL}, #FPR
                             TP.NULL/{TN.NULL+FP.NULL}, #TPR
                             TP.NULL/{TP.NULL+FP.NULL}, #PPV
                             FN.NULL/{FN.NULL+TN.NULL}, #FOR
                             TN.NULL/{TN.NULL+FN.NULL}, #NPV
                             FP.NULL/{FP.NULL+TP.NULL}, #FDR
                             {{TP.NULL/{TN.NULL+FP.NULL}}/{FP.NULL/{FP.NULL+TN.NULL}}}, #PLR
                             FN.NULL/{FN.NULL+TP.NULL}, #FNR
                             {{FN.NULL/{FN.NULL+TP.NULL}}/{TN.NULL/{TN.NULL+FP.NULL}}}, #NLR
                             {{{TP.NULL}*{TN.NULL}}/{{FN.NULL}*{FP.NULL}}}, #DOR
                             0)] #logDOR holder
foodf[,logDOR.NULL:=log({DOR.NULL})]

##  Resave the CSV:
write.csv(foodf,
          file = paste0("E:/Research/BSGMe/CountryLevelPixelComparisons_Extrapolation_",
                        in_series,"_2010-2015_w_NULL.csv"),
          row.names = F)
...
in_series <- "Observed"
data_tag <- "esa_cls190"
root <- paste0("E:/Research/BSGMe/",in_series," Input Validation/")
output_directory <- paste0(root)
##  Declare the model names and the period list we have in each:
model_list <- list("2010-2015" = list("1"=c(2010,2015)))
...
for(i in iso3){
  ...
  for(m in 1:length(model_list)){
    print(paste0("Working on ",i," ",names(model_list)[m], " ", in_series))
    ##  Retrieve the model's year tag(s):
    year_tag <- names(model_list[m])
    out_root <- paste0(output_directory,"prj_",year_tag,"_",i,"/derived/")
    
    ## Retrieve the aggregated contingency RDS format data.table:
    foodt <- readRDS(paste0(out_root,i,"_",data_tag,"_admin_level_contingency_data_",
                            year_tag,
                            ".RDS"))
    ##  Add a model column that gives a model specific tag:
    foodt[,MODEL := year_tag]
    ...
    if(!file.exists(Sys.glob(paste0(out_root,i,"_",data_tag,
                                    "NULLMODEL_admin_level_contingency_data_2010-2015.RDS")))){
    ...
    nulldt <- readRDS(Sys.glob(paste0(out_root,i,"_",data_tag,
                                      "NULLMODEL_admin_level_contingency_data_2010-2015.RDS")))
    ##  Ensure the GID and YEAR are numeric:
    ...
    ##  Change the names to the desired format:
    # names(nulldt)[c(7,8,9,10,11,14,17)] <- c("RECALL.NULL","PRECISION.NULL",
    #                                           "F1.NULL","SPECIFICITY.NULL",
    #                                           "OVER.ACC.NULL","QUANT.NULL",
    #                                           "ALLOC.NULL")
    ##  Merge the columns over:
    foodt[nulldt,
          names(nulldt)[c(2,3,4,5,6,7,10,11,12,13,14,17,20)] := nulldt[,.SD,.SDcols = c(2,3,4,5,6,7,10,11,12,13,14,17,20)]]
  ...
  saveRDS(main_dt,
          file = paste0(output_directory,"AdminLvl_Contingencies_MultiModels_",
                        in_series,i,"_",year_tag,"_w_Null.RDS"))
}
##  Combine all the countries into a single file:
for(i in 1:length(iso3)){
  if(i == 1){
    # foodt <- readRDS(file = paste0(output_directory,"output/prj_",year_tag,"_",
    #                                iso3[i],"/derived/AdminLvl_Contingencies_MultiModels_",
    #                                iso3[i],"_",year_tag,".RDS"))
    foodt <- readRDS(file = paste0("E:/Research/BSGMe/",in_series," Input Validation/","AdminLvl_Contingencies_MultiModels_",in_series,
                                   iso3[i],"_",year_tag,"_w_Null.RDS"))
  }else{
    # tmp <- readRDS(file = paste0(output_directory,"output/prj_",year_tag,"_",
    #                              iso3[i],"/derived/AdminLvl_Contingencies_MultiModels_",
    #                              iso3[i],"_",year_tag,".RDS"))
    tmp <- readRDS(file = paste0("E:/Research/BSGMe/",in_series," Input Validation/","AdminLvl_Contingencies_MultiModels_",in_series,
                                 iso3[i],"_",year_tag,"_w_Null.RDS"))
    foodt <- rbind(copy(foodt),copy(tmp))
  }
}
...
foodt[,c("FPR",
         ...
         "logDOR") := .(FP/{FP+TN}, #FPR
                        ...
                        {{TP/{TN+FP}}/{FP/{FP+TN}}}, #PLR
                        FN/{FN+TP}, #FNR
                        {{FN/{FN+TP}}/{TN/{TN+FP}}}, #NLR
                        {{{TP}*{TN}}/{{FP}*{FN}}}, #DOR
                        0)] #logDOR holder

foodt[,logDOR:=log({DOR})]

##  Create NULL Model results:
# foodt[, c("TP.NULL","FP.NULL",
#           "FN.NULL","TN.NULL"):=
#         .(0,0,{TP+FN},TN)]

##  Calculate the corresponding null model metrics:
foodt[,c("FPR.NULL",
         "TPR.NULL",
         "PPV.NULL",
         "FOR.NULL",
         ...
         "logDOR.NULL") := .(FP.NULL/{FP.NULL+TN.NULL}, #FPR
                             ...
                             FN.NULL/{FN.NULL+TP.NULL}, #FNR
                             {{FN.NULL/{FN.NULL+TP.NULL}}/{TN.NULL/{TN.NULL+FP.NULL}}}, #NLR
                             {{{TP.NULL}*{TN.NULL}}/{{FP.NULL}*{FN.NULL}}}, #DOR
                             0)] #logDOR holder
foodt[,logDOR.NULL:=log({DOR.NULL})]
# foodt[,logDOR.DIFF := {logDOR - logDOR.NULL}]

write.csv(foodt, 
          file = paste0(root, "AdminLevelPixelComparisons_2010-2015.csv"),
          row.names = F)
...
esamods <- grep(".*/(prj_2000-2005-2010_VNM|prj_2000-2005-2010_UGA|prj_2000-2005-2010_PAN|prj_2000-2005-2010_CHE)/tmp/bsgmfit_final_.*.Rdata",esamods,value= T)
...
##  Save the file:
saveRDS(impdt, file = paste0("E:/Research/BSGMe/aggregate_variable_importance_BSGMe_2011-2015_CHE_PAN_UGA_VNM.RDS"))



##    Facted VarImpPlot-type Diagram (by Country and Datasource)
shapes <- c(0,1,2,6,7)
importance_dt <- readRDS(paste0("E:/Research/BSGMe/aggregate_variable_importance_BSGMe_2011-2015_CHE_PAN_UGA_VNM.RDS"))
names(shapes) <- unique(importance_dt$PLOTGROUP)
root <- "E:/Research/BSGMeGeneral/"
